...
// FireAngular
// TODO: angular-renderer
const { getAngularifyCode } = require("./angularify-code");
 ...
 *
 * FireAngular is appending a new 'Angularify' toolbar button
 */
...
/** @lends ConsoleOverlay */
{
  ...
  overlayId: "fireAngularConsoleOverlay",
  panelId: "webconsole",
  ...
  destroy: function() {
    ...
    removeSheet(win, "chrome://fireangular/skin/fireangular.css", "author");
  },
  ...
  onBuild: function(options) {
    ...
    // is used to load latest Angular into the current page.
    let button = new ToolbarButton({
      id: "fireangular-angularify",
      ...
      label: "fireangular.Angularify.label",
      tooltiptext: "fireangular.Angularify.tip",
      command: this.onAngularify.bind(this)
    });
  ...
  onAngularify: function() {
    ...
    // Evaluate Angular script in the current page.
    let script = getAngularifyCode();
    webConsoleClient.evaluateJSAsync(script, response => {
      if (response.error) {
        TraceError.sysout("ConsoleOverlay.onAngularify; ERROR " +
          response.error, response);
      }
      else if (response.exception !== null) {
        TraceError.sysout("ConsoleOverlay.onAngularify; EXCEPTION " +
          response.exception, response);
      }
      else {
        Trace.sysout("ConsoleOverlay.onAngularify; DONE " +
